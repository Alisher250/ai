...
from utils.general import check_img_size, check_requirements, check_imshow, non_max_suppression, non_max_suppression_kpt, apply_classifier, \
    scale_coords, scale_coords_kpt, xyxy2xywh, strip_optimizer, set_logging, increment_path, save_one_box
from utils.plots import colors, plot_one_box, plot_one_box_kpt
...
def detect(opt):
    ...
    print((save_dir / 'labels' if (save_txt or save_txt_tidl) else save_dir))
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        print(pred[...,4].max())
        # Apply NMS
        pred = non_max_suppression_kpt(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms, kpt_label=kpt_label)
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                scale_coords_kpt(img.shape[2:], det[:, :4], im0.shape, kpt_label=False)
                scale_coords_kpt(img.shape[2:], det[:, 6:], im0.shape, kpt_label=kpt_label, step=3)
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:,:6])):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        plot_one_box_kpt(xyxy, im0, label=label, color=colors(c, True), line_thickness=opt.line_thickness, kpt_label=kpt_label, kpts=kpts, steps=3, orig_shape=im0.shape[:2])
                        if opt.save_crop:
